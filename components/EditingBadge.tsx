...
const EditingBadge: React.FC<EditingBadgeProps> = ({
  ...
  return (
    <div ref={ref} className="relative z-10">
      <button
        onClick={() => setOpen(v => !v)}
        className={`flex items-center gap-1.5 px-2.5 py-1.5 min-h-[44px] rounded-full text-sm font-medium backdrop-blur-md transition-all ${badgeClass}`}
      ...
      >
        ...
        <span>{label}</span>
        {chainCapped && <span className="text-xs opacity-80">· chain full</span>}
        ...
        <div className="mt-1.5 w-64 rounded-lg bg-zinc-900/95 backdrop-blur-md border border-zinc-800 shadow-xl overflow-hidden">
          {lastTool && (
            <div className="px-3 py-2 border-b border-zinc-800">
              <div className="text-xs uppercase tracking-wider text-zinc-500 mb-0.5">Last edit</div>
          ...
          {chainCapped && onCommitAndContinue && (
            ...
            >
              ...
              <span className="text-left">
                ...
                <span className="block text-xs text-zinc-400 leading-tight mt-0.5">Lock in current result as new base. Prevents further quality drift.</span>
              </span>
            </button>
          )}
          <button
