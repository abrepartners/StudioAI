...
function trimToSentence(text: string, max: number): string {
  ...
  // Require the boundary to be reasonably deep so we don't gut the whole list.
  if (lastStop > max * 0.5) return slice.slice(0, lastStop + 1);
  // No good boundary — cut at the last whole word.
...
export function buildStagingAssignment(
  ...
  // Scaffold = the load-bearing content (additive intent + HARD PRESERVATION
  // RULES), emitted FIRST so it always survives the 2560 cap. The per-pack DNA
  // fields (materials/palette/arrangement) are clamped — some packs write very
  // long arrangement notes that would otherwise eat the furniture budget — and
  // the secondary guidance blocks are kept terse. The verbose furniture list
  // goes LAST and is the only content that gets trimmed.
  const materials = trimToSentence(pack.materials, 200);
...
FURNITURE TO ADD: This is an outdoor/utility space — add only appropriate outdoor furniture and decor for the style. No indoor furniture.`;
  }

  // Budget the furniture list against what's left under the cap, then trim it
  // at a clean sentence boundary so it never ends mid-item. The clamped
  // scaffold lands ~1550 chars, leaving ~1000 for furniture — enough for the
  // primary pieces of every room; only the long styling tail gets clipped.
  const header = "\n\nFURNITURE TO ADD:\n";
  const budget = STAGING_EDIT_INSTRUCTION_CAP - scaffold.length - header.length;
  const furnitureTrimmed = trimToSentence(furniture, Math.max(0, budget));
...
export const STYLE_PACKS: Record<string, StylePack> = {
  ...
  "mid-century": {
    ...
    materials:
      "Walnut and teak wood with visible grain, molded plywood, genuine or faux leather in cognac/caramel, wool upholstery, brass legs and pulls, terrazzo accents.",
    palette:
      "Warm walnut, cognac leather, mustard or olive accent (ONE per room), cream, warm white. Pops of teal or burnt orange in pillows or art only.",
    antiPatterns:
      "No heavy overstuffed furniture. No matching wood tones everywhere — mix walnut with teak or oak. No modern/industrial mashup. Keep it warm, not cold.",
    rooms: {
      "Living Room":
  ...
  scandinavian: {
    ...
    materials:
      "Pale birch and ash wood, wool and sheepskin, matte white ceramics, brushed steel, cotton canvas, unglazed pottery.",
    palette:
      "Warm white, pale gray, birch blonde, soft black accents. One muted accent per room — blush, dusty blue, or sage. Never saturated.",
    antiPatterns:
      "No heavy curtains. No ornate furniture. No dark wood. No clutter — every object earns its place. Fewer pieces, more space, but each piece is thoughtfully styled.",
    rooms: {
      "Living Room":
  ...
  minimalist: {
    ...
    materials:
      "Monolithic forms — solid wood slabs, cast concrete, smooth plaster, matte metals, fine-weave upholstery. No visible hardware. No ornament.",
    palette:
      "White, warm gray, charcoal, natural wood (ONE tone). One accent color maximum — muted, not bright.",
    antiPatterns:
      "No decorative pillows beyond what is specified. No gallery walls. No tchotchkes. Every single object is intentional and sculptural. A minimalist room is curated, not bare — each piece has visual weight and purpose.",
    rooms: {
      "Living Room":
        "One low-profile sofa in warm gray or off-white with clean monolithic form — 2 pillows maximum in the same tone. One sculptural coffee table (cast concrete, solid wood slab, or smooth stone) — styled with one single sculptural object (ceramic vessel or stone orb). One accent chair with architectural lines if the room is large. One arc floor lamp in matte black or brushed nickel. Solid-color area rug with clean edges in warm gray or cream. One oversized piece of artwork on the focal wall — abstract, minimal, or monochromatic. One tall sculptural floor plant (single-stem bird of paradise or architectural succulent) in a matte planter. No side tables unless the room demands it — if used, one only, with nothing on it.",
      Bedroom:
      ...
      "Dining Room":
        "Monolithic dining table (solid wood slab or cast concrete with clean legs). Matching chairs with architectural form — no mix, no cushions. One single object at the center of the table (sculptural ceramic, stone bowl, or single branch in a minimal vase). One piece of artwork on the nearest wall. Nothing else.",
      Office:
        "Slab desk (solid wood or concrete). One architectural task chair. One sculptural desk lamp. One single object on the desk (stone paperweight, ceramic vessel, or closed notebook). One floating shelf with 3-4 books aligned precisely. Nothing else.",
    },
  },
...
export const VELLUM_STYLE_KEYS = [
  "contemporary",
  "mid-century",
  "coastal",
  "farmhouse",
