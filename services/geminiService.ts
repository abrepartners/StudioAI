...
=== SHADOW / REFLECTION EXTENSION — MANDATORY ===
The mask covers each object's BODY. It does NOT cover the shadow the object casts, the reflection of the object in glossy surfaces, the soft light halo the object sheds into the surrounding area, or dust/lint accumulated at the object's base. ALL of these MUST be erased together with the object. A shadow left behind without its object is the clearest possible "AI-edited here" tell — leaving one is an automatic failure.

For EVERY masked object you erase, you MUST ALSO erase:
  1. ITS CAST SHADOW — the dark elongated smudge on the floor, wall, or adjacent surface pointing away from the light source. Erase the ENTIRE shadow all the way to its natural soft edge, even if that extends 100-150 pixels beyond the mask boundary. Do NOT stop at the mask edge.
  2. ITS CONTACT SHADOW — the denser dark band directly under/against the object where it meets the floor or wall. Usually 20-60 pixels wide. Must be erased completely.
  3. ITS REFLECTION — in any mirror, window glass, TV screen, framed picture glass, polished countertop, glossy tile floor, stainless appliance. Replace with what that surface would show if the object had never been there.
  4. ITS AMBIENT DARKENING HALO — the subtle grey/brown tint in surrounding pixels caused by the object blocking ambient light. Blend back to the clean surrounding surface's tone.
  5. ANY BASE DEBRIS — scuff marks, dust, lint, rug dents, flattened carpet fibers, water rings, wear patterns. Restore the pristine surface.

MENTAL MODEL: Do not think in pixel counts. Think: "what pixels would look different if this object had NEVER been in this room?" Every such pixel must change. When in doubt about whether a darker-than-surrounding pixel near the mask is shadow or clean surface, ASSUME SHADOW and erase it.

SELF-CHECK BEFORE RETURNING: Scan every area where you erased an object. Is there any dark smudge, grey residue, soft halo, scuff, or shadow-shape remaining? If yes, you are NOT DONE — repaint that area with the surrounding clean surface pattern and texture until the area is indistinguishable from the rest of the floor/wall.

BLACK regions of the mask that are FAR from any white (masked) area must remain bit-identical to IMAGE 1.
