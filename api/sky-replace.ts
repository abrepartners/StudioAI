 ...
 *
 * Input (POST JSON):
 *   { imageBase64: string, style: 'blue' | 'dramatic' | 'golden' | 'stormy', skipUpscale?: boolean }
 *
 * Output (200 JSON):
...
const VALID_STYLES: ReadonlyArray<SkyStyle> = [
  "blue",
  "dramatic",
  "golden",
  "overcast",
...
const STYLE_SKY_DESCRIPTIONS: Record<SkyStyle, string> = {
  blue: "a vivid, clean deep blue sky with a few soft scattered cumulus clouds. Bright, sunny, MLS-ready daytime look",
  dramatic:
...
const ATMOSPHERIC_LIGHTING: Record<SkyStyle, string> = {
  ...
  dramatic:
    "Moodier contrast with directional light through cloud breaks — slightly lower ambient, stronger highlights.",
  golden:
    "Warm golden-hour side-lighting — warm tones on sun-facing surfaces, cool shadows.",
  overcast:
    "Soft, even diffused light — minimal shadows, gentle illumination on all surfaces.",
  stormy:
...
export default async function handler(req: any, res: any) {
  ...
  if (!VALID_STYLES.includes(style)) {
    json(res, 400, { ok: false, error: `Invalid style: ${style}` });
    return;
  }
  ...
  try {
    console.log(
      `[sky-replace] Starting Nano Banana sky replacement (${style})`,
    );
    ...
    if (!skipUpscale) {
      ...
      try {
        ...
        console.warn(
          `[sky-replace] Pruna failed: ${upErr?.message} — using un-upscaled`,
        );
      }
    }
