...
export const createChatSession = (): Chat => {
  return getAI().chats.create({
    ...
    config: {
      ...
      - Virtual Twilight: Converting day exterior shots to golden-hour dusk (handled by Flux 2 Pro)
      - Declutter/Cleanup: Removing personal items and clutter to reveal clean spaces (handled by Flux 2 Pro)
...
=== SHADOW / REFLECTION EXTENSION ===
The mask covers each object's body, but NOT the shadow it casts on the floor/wall or its reflection in a glossy surface. When you erase a masked object, ALSO erase the cast shadow and any reflection of that object. Specifically:
  - A soft shadow extending from the base of a masked object onto the floor or wall → erase and inpaint the surface.
  - The reflection of a masked object in a mirror, window glass, TV screen, polished counter, or glossy floor → erase and inpaint.
  - Light-color halos around the masked object (indirect bounce light) → blend back into the surrounding surface.

If you only erase the mask region and leave the shadow behind, the output reads as an obvious "object removed here" giveaway. That's a failure. Extend the erasure up to 40-60 pixels (at input resolution) beyond the mask edge WHERE a shadow or reflection clearly belongs to a masked object.

BLACK regions far from any mask must remain bit-identical to IMAGE 1.
