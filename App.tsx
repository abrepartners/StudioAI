...
const App: React.FC = () => {
  ...
  const handleGenerate = async (prompt: string, highRes = false) => {
    ...
    try {
      ...
      const sendMask = activePanel === 'cleanup' && cleanupMode === 'precision' ? maskImage : null;
      const resultImage = await generateRoomDesign(sourceImage, prompt, sendMask, highRes, activePanel === 'cleanup' ? cleanupMode : null);
            ...
            <div className="mx-auto w-full max-w-6xl space-y-4">
              <div className="premium-surface-strong rounded-[2rem] p-2 sm:p-3">
                <div className="relative overflow-hidden rounded-[1.5rem] border panel-divider bg-[var(--color-bg-deep)] aspect-[4/3] sm:aspect-video">
                  {activePanel === 'cleanup' && cleanupMode === 'precision' ? (
                    <MaskCanvas
