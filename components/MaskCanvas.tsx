...
// D14 — memory fix.
// Previous implementation pushed a full `ImageData` raster per stroke onto
// `historyRef`. For a 2048x2048 canvas each snapshot is ~16 MB; a 50-stroke
// session could pin 800 MB+. We now store vector stroke descriptors and
// replay-on-undo by clearing + redrawing from index 0..historyIndex.
//
// Memory per stroke: ~24 B/point × avg-points-per-stroke (typically 50-300).
// Even a 1000-stroke session caps at a few MB — three orders of magnitude less
// than the raster approach.
interface MaskStroke {
  points: Array<{ x: number; y: number }>;
  brushSize: number;
  /** Whether the stroke auto-closed into a filled region (loop detection). */
  closed: boolean;
}
...
const MaskCanvas: React.FC<MaskCanvasProps> = ({ imageSrc, onMaskChange, isActive }) => {
  ...
  // D14: stroke-list history instead of raster snapshots. Index -1 = blank.
  ...
  // D14: re-render the canvas from the stroke list up to historyIndexRef.
  // Called on undo/redo; strokes 0..historyIndex are replayed onto a cleared
  // canvas. Small strokes replay in under a millisecond; worst-case (1000
  // strokes × 300 points) is still sub-50ms on modern hardware.
  const renderFromHistory = useCallback(() => {
    ...
    // Restore the live brush stroke settings for in-progress drawing.
    ctx.lineWidth = brushSize;
  }, [getContext, brushSize]);

  const pushStroke = useCallback((stroke: MaskStroke) => {
    // Trim future history if we're not at the end (undo then new stroke branches).
  ...
  const undo = useCallback(() => {
    // Blank state is historyIndex === -1 (no strokes rendered).
    if (historyIndexRef.current < 0) return;
    historyIndexRef.current -= 1;
    renderFromHistory();
    forceUpdate(n => n + 1);
    exportMask();
  }, [renderFromHistory, exportMask]);
  ...
  const stopDrawing = () => {
    ...
    if (ctx && canvas) {
      ...
      // D14: push vector stroke descriptor instead of a raster snapshot.
      if (path.length > 0) {
        pushStroke({ points: path.slice(), brushSize, closed });
      }
  ...
  const clearCanvas = () => {
    ...
    if (canvas && ctx) {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      // D14: reset history — blank state is index -1.
      historyRef.current = [];
      historyIndexRef.current = -1;
      forceUpdate(n => n + 1);
      onMaskChange(null);
