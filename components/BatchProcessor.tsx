...
// NOTE: legacy surface (dead /legacy route). The 'stage' action used the
// browser-side Gemini image path (generateRoomDesign), which is purged. The
// cleanup/twilight/sky actions already run server-side via Replicate services
// (fluxCleanup / fluxTwilight / nanoSky) and are unaffected.
...
import {
  CLEANUP_COMPOSITE_OPTIONS,
  shouldSkipCompositeForTool,
} from "../utils/compositeProfiles";
...
async function postProcessBatchOutput(
  ...
  const compositeOptions =
    action === "cleanup" ? CLEANUP_COMPOSITE_OPTIONS : undefined;
  try {
    return await compositeStackedEdit(prior, sharpened, {
      format: fmt,
      ...compositeOptions,
    });
  } catch (err) {
    console.warn(
      "[BatchProcessor] composite failed, using sharpened raw:",
      err,
    );
    return sharpened;
  }
...
const processImage = async (
  ...
  switch (img.action) {
    case "stage": {
      // DISABLED on this legacy surface: virtual staging used a browser-side
      // Gemini call (generateRoomDesign), which is purged. Staging now lives in
      // the Vellum editor and runs server-side via Replicate. This dead /legacy
      // batch path fails loudly instead of silently invoking a removed Gemini
      // path. roomType referenced only to keep the lint happy on this branch.
      void roomType;
      throw new Error(
        "Batch staging is disabled — use the editor (server-side Replicate).",
      );
    }
    case "cleanup": {
      const { resultBase64 } = await fluxCleanup(img.base64, roomType);
      // Skip composite on the Nano Banana cleanup path — compositing the
      // original back over the result would create ghost artifacts on the
      // removed items. Sharpen + resize only.
      const chainEnabled =
        new URLSearchParams(window.location.search).get("chain") !== "0";
      const fmt: "png" | "jpeg" = chainEnabled ? "png" : "jpeg";
    ...
    case "twilight": {
      const { resultBase64 } = await fluxTwilight(
        img.base64,
        "golden",
        "sunset",
      );
    ...
    case "sky": {
      const { resultBase64 } = await nanoSky(img.base64, "blue");
      return await postProcessBatchOutput(resultBase64, img.base64, "sky");
    }
    case "export": {
...
const BatchProcessor: React.FC<BatchProcessorProps> = ({
  ...
  const [results, setResults] = useState<BatchResult[]>(() =>
    initialResults && initialResults.length === imagesRef.current.length
      ...
      : imagesRef.current.map((img) => ({
          ...
          action: img.action,
          status:
            img.action === "export" ? ("done" as const) : ("pending" as const),
          saved: false,
        })),
  );
      ...
      if (img.action === "export") {
        void (async () => {
          ...
          setResults((prev) =>
            prev.map((r) =>
              r.id === img.id
                ? { ...r, generatedImage: img.base64, saved: true }
                : r,
            ),
          );
        })();
  ...
  const processQueue = useCallback(async () => {
    ...
    try {
      ...
      const processOne = async (img: BatchImage) => {
        ...
        const actionAtDispatch: BatchAction =
          currentResult?.action ?? img.action;
        const dispatchImg: BatchImage =
          actionAtDispatch === img.action
            ? img
        ...
        setResults((prev) =>
          prev.map((r) =>
            r.id === img.id
              ? { ...r, status: "processing", action: actionAtDispatch }
              : r,
          ),
        );
        ...
        try {
          ...
          const cleanupQuality =
            actionAtDispatch === "cleanup"
              ...
              : undefined;
          if (cleanupQuality) {
            trackCleanupRisk(cleanupQuality.risk, {
              source: "batch",
              imageId: img.id,
            });
          }
  ...
  const retryFailed = useCallback(() => {
    const failedIds = results
      ...
      .map((r) => r.id);
    const hasCleanupRetry = results.some(
      (r) => r.status === "error" && r.action === "cleanup",
    );
    if (hasCleanupRetry) {
      trackEvent("cleanup_retried", {
        source: "batch",
  ...
  const doneResults = results.filter(
    (r) => r.status === "done" && r.generatedImage,
  );
  useEffect(() => {
    setSelectedIds((prev) => {
  ...
  const allSelected =
    doneResults.length > 0 && doneResults.every((r) => selectedIds.has(r.id));
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    try {
      ...
      setFatalError(
        "Download failed. Please try again or use Export with preset.",
      );
    } finally {
      setExporting(false);
  ...
  const lightboxResult = lightboxId
    ? results.find((r) => r.id === lightboxId)
    : null;
    ...
    <div className="space-y-4">
      ...
      <div className="premium-surface rounded-2xl p-4 space-y-3">
              ...
              >
                {exporting ? (
                  ...
                  <>
                    ...
                    {selectedIds.size > 0 &&
                    selectedIds.size < doneResults.length
                      ? `${selectedIds.size}`
                      : "All"}
                  </>
                )}
        ...
        {(pendingCount > 0 || errorCount > 0) && (
          <div className="flex items-center gap-1.5 flex-wrap border-t border-white/5 pt-2.5">
            ...
            {(["stage", "cleanup", "twilight", "sky"] as BatchAction[]).map(
              (action) => {
      ...
      {allDone && doneResults.length > 0 && (
        <div className="flex items-center justify-between px-1 text-xs text-[var(--color-text)]/70">
          ...
          >
            <span
              className={`w-3.5 h-3.5 rounded border flex items-center justify-center ${allSelected ? "bg-[var(--color-primary)] border-[var(--color-primary)]" : "border-white/30"}`}
            >
          ...
          <span className="text-xs text-[var(--color-text)]/50">
            Click a result to preview · Esc closes
          </span>
        </div>
      )}
            ...
            >
              ...
              {result.status === "error" && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-1 p-2">
                  ...
                  <span className="text-2xs text-[#FF375F] text-center truncate w-full">
                    {result.error}
                  </span>
                </div>
              )}
              ...
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-1.5 flex items-center justify-between">
                <span className="text-2xs font-semibold text-white truncate">
                  {result.roomType}
                </span>
                {result.status === "done" && (
              ...
              {result.cleanupQuality && result.action === "cleanup" && (
                <div
                  className={`absolute bottom-6 left-1.5 px-1.5 py-0.5 rounded text-2xs font-bold uppercase tracking-wider ${
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-white/10">
            <div className="flex items-center gap-3 text-xs text-white">
              <span className="inline-flex items-center gap-1.5 font-semibold">
                <span
                  ...
                  style={{
                    backgroundColor: ACTION_LABELS[lightboxResult.action].color,
                    color: "#000",
                  }}
                >
                  {ACTION_LABELS[lightboxResult.action].icon}{" "}
                  {ACTION_LABELS[lightboxResult.action].label}
                </span>
                {lightboxResult.roomType}
              </span>
              <span className="text-xs text-white/50">
                {doneResults.findIndex((r) => r.id === lightboxResult.id) + 1}{" "}
                of {doneResults.length}
              </span>
            </div>
            <div
            ...
            >
              <button
                ...
                onClick={() => {
                  onLoadImage(
                    lightboxResult.originalImage,
                    lightboxResult.generatedImage!,
                  );
                  setLightboxId(null);
                }}
          ...
          >
            <div className="relative w-full max-w-2xl">
              <img
                ...
                decoding="async"
              />
              <span className="absolute bottom-2 left-2 bg-black/80 text-xs font-bold text-white uppercase px-2 py-1 rounded">
                Before
              </span>
            </div>
            <div className="relative w-full max-w-2xl">
              <img
                src={
                  lightboxResult.generatedImage.startsWith("data:")
                    ? lightboxResult.generatedImage
                    : `data:image/jpeg;base64,${lightboxResult.generatedImage}`
                }
                alt="After"
                className="w-full h-auto rounded-xl"
                loading="lazy"
                decoding="async"
              />
              <span className="absolute bottom-2 left-2 bg-[var(--color-primary)]/90 text-xs font-bold text-white uppercase px-2 py-1 rounded">
                After
              </span>
            </div>
          </div>
