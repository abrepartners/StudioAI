...
import { Users, Plus, Trash2, Loader2, Building2, AlertCircle } from 'lucide-react';
...
const ManageTeam: React.FC<ManageTeamProps> = ({ adminEmail }) => {
      ...
      return (
        <div className="space-y-3">
          ...
          <p className="text-xs text-[var(--color-text)]/70">
            Set up a brokerage to add agents and manage their access to StudioAI Pro.
          </p>
          <button
            type="button"
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="cta-secondary rounded-xl py-2.5 text-sm font-semibold"
          >
            Cancel
          </button>
          <button
  ...
  // Brokerage exists — show management
          ...
          {agents.map((agent) => (
            ...
            >
              <div className="min-w-0">
                {agent.name && (
                  <p className="text-xs font-semibold text-[var(--color-ink)] truncate">{agent.name}</p>
                )}
