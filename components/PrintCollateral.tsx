 ...
 *   <PrintCollateral open onClose={…} images={…} listingMeta={…} />
 *
 * Output path is unchanged: @react-pdf/renderer dynamic import → toBlob → download.
 *
...
async function generatePDF(
  ...
  const primary = brandKit.primaryColor || "#d8c79a";
          ...
          <View style={styles.body}>
            ...
            <View style={styles.specs}>
              <Text style={styles.specItem}>
                <Text style={styles.specValue}>{data.beds}</Text> Beds
              </Text>
              <Text style={styles.specItem}>
                <Text style={styles.specValue}>{data.baths}</Text> Baths
              </Text>
              <Text style={styles.specItem}>
            ...
            <View style={styles.agentCard}>
              {brandKit.headshot && (
                <Image src={brandKit.headshot} style={styles.agentPhoto} />
              )}
              <View>
        ...
        <Page size="LETTER" style={styles.page}>
          <View style={{ ...styles.body, paddingTop: 40 }}>
            ...
            {Array.from({ length: 15 }, (_, i) => (
              ...
              >
                <Text style={{ flex: 2, fontSize: 9, color: "#9CA3AF" }}>
                  Name
                </Text>
                <Text style={{ flex: 2, fontSize: 9, color: "#9CA3AF" }}>
                  Email
                </Text>
                <Text style={{ flex: 1.5, fontSize: 9, color: "#9CA3AF" }}>
                  Phone
                </Text>
                <Text style={{ flex: 1, fontSize: 9, color: "#9CA3AF" }}>
                  Agent?
                </Text>
              </View>
            ))}
...
const PrintCollateral: React.FC<PrintCollateralProps> = ({
  open,
  images,
  projectName,
  listingMeta,
  ...
  const [selectedTemplate, setSelectedTemplate] =
    useState<TemplateType>("flyer");
  const [isGenerating, setIsGenerating] = useState(false);
  const [downloadDone, setDownloadDone] = useState(false);

  // Build the internal ListingData shape from the [GEN-PROPS] contract.
  // Photos come straight from the passed-in (already refined) images; the
  // contract carries no description, so the PDF templates fall back to their
  // built-in placeholder copy.
  const listingData = useMemo<ListingData | null>(() => {
    const photos = (images || []).map((img) => img.dataUrl).filter(Boolean);
    if (photos.length === 0) return null;
    return {
      address: listingMeta?.address || projectName || "Property Address",
      beds: listingMeta?.beds ?? 0,
      baths: listingMeta?.baths ?? 0,
      sqft: listingMeta?.sqft ?? 0,
      price: listingMeta?.price ?? 0,
      description: "",
      photos,
    };
  }, [images, listingMeta, projectName]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        >
          <Printer className="w-5 h-5 text-[#d8c79a]" />
          Print Collateral
        </h3>
        <p className="text-zinc-400 text-sm mt-0.5">
          {projectName
            ? `${projectName} — branded flyers, open house sheets, and postcards as print-ready PDFs`
            ...
            <button
              ...
              className={`p-3 rounded-xl border text-left transition-all duration-200 ${
                selectedTemplate === t.type
                  ? "bg-zinc-800 border-[#d8c79a] ring-1 ring-[#d8c79a]/30"
                  : "bg-zinc-800/50 border-zinc-700 hover:border-zinc-600"
              }`}
            >
              <Icon
                className={`w-5 h-5 mb-2 ${selectedTemplate === t.type ? "text-[#d8c79a]" : "text-zinc-500"}`}
              />
      ...
      {!hasBrandKit && (
        <div className="flex items-center gap-2 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2">
          ...
          <span className="text-xs text-zinc-400">
            Set up your Brand Kit in Settings for branded collateral with your
