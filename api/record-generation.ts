...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Consumption accounting: callers pass the cost of a successful generation
    // (fractional for whiten, >1 for batches). Default to 1 for legacy callers.
    ...
    if (searchRes.data && searchRes.data.length > 0) {
      ...
      const created = await fetch("https://api.stripe.com/v1/customers", {
        ...
        body: `email=${encodeURIComponent(email)}&metadata[generation_period]=${getCurrentDay()}&metadata[generations_used]=${amount}`,
      }).then((r) => r.json());
    ...
    if (plan === "starter") {
      ...
      let currentCount = parseInt(
        customer.metadata?.generations_used || "0",
        10,
      );
