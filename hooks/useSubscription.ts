...
export function useSubscription(userEmail: string | null) {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        const lifetimeFreeGensUsed =
          data.lifetimeFreeGensUsed ?? data.generationsUsed ?? 0;
        const lifetimeFreeGensCap =
          data.lifetimeFreeGensCap ?? FREE_LIFETIME_CAP;
  ...
  const recordGeneration = useCallback(
    async (
      ...
      // Optimistic update — increment ledger by `amount` (may be fractional).
      setState((prev) => {
        const newCount = prev.generationsUsed + amount;
        const newLifetimeUsed =
          prev.plan === "free"
            ? Math.min(
                prev.lifetimeFreeGensCap,
                prev.lifetimeFreeGensUsed + amount,
              )
        ...
        return {
          ...
          canGenerate:
            hasUnlimitedGeneration(prev.plan) ||
            prev.generationsLimit === -1 ||
            newCount < prev.generationsLimit,
        };
      });
      ...
      try {
        ...
        if (data.ok && data.generationsUsed !== -1) {
          setState((prev) => ({
            ...
            lifetimeFreeGensUsed:
              data.lifetimeFreeGensUsed ?? prev.lifetimeFreeGensUsed,
            canGenerate:
              hasUnlimitedGeneration(prev.plan) ||
              prev.generationsLimit === -1 ||
              data.generationsUsed < prev.generationsLimit,
          }));
        }
      } catch {
  ...
  const buyCredits = useCallback(
    async (pack: "starter" | "pro_pack" | "agency", userId: string) => {
      if (!userEmail) return;
      try {
        const res = await fetch("/api/stripe-checkout", {
          method: "POST",
